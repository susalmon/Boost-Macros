...
The macro writes the information inputted through the GUI onto a text file in the '\Users\Public\AltItemUser\' folder which allows all local accounts to read it. There are 3 buttons in the macro: Submit, Stop and CloseALL.
The 'Submit' button submits the information from the GUI into the text file to be interpreted. The 'Stop' button sets all of timer values to 0, this nearly instantly stops the listener from using more items. The CloseALL button erases all information on the text file which closes all of the instances of MacroCommListener.ahk and MacroCommInput.ahk. All the delay values are currently in milliseconds.
